...
### 출처 보강·중복 처리 가드 (2026-06-12 추가, 실제 사고 기반)
보강·dedup·정규화 시 아래를 지킨다. 셋 다 과거 오류에서 도출:

1. **dedup은 문자열이 아니라 권위 순위로.** 같은 표기가 이미 있어도 출처가 더 약하면(애그리게이터 Artnet/MutualArt/Invaluable/Artsy) 새 기관 1차 출처로 **교체**한다 — "이미 있음"으로 건너뛰지 않는다. 권위 사다리: 기관 1차 ≥ 문헌 > 언론 > (애그리게이터=제외). *사고: 박수근 'Park Sookeun'이 Invaluable(제외 대상)에만 근거했는데 MMCA 영문 페이지를 "중복"이라 버릴 뻔함.*
2. **변이집에서 정규화가 카탈로그 대상을 뭉개지 않게.** 하이픈·대소문자·띄어쓰기(Hui-dong vs Huidong, Park vs PARK, Soo Keun vs Sookeun)는 **이 어휘집이 기록하려는 변이 그 자체**다. novelty 판정(새 변이인가?)은 하이픈·공백을 지우지 말고 **표기 그대로 비교**한다. 정규화는 표시용 casing에만, 그 사실은 note에 남긴다. *사고: dedup이 하이픈을 지워 'Ko Huidong'을 'Ko Hui-dong'과 같다고 폐기.*
3. **자동 dedup/필터는 버린 것을 로그하고 눈으로 확인한 뒤 결론낸다.** "수확 0 / 더 없음 / 한계 도달" 같은 **부정·종결 결론은, 필터가 버린 항목을 들여다보고 + 다른 방법(예: Naver-Playwright)으로 교차확인하기 전엔 사용자에게 말하지 않는다.** 0건은 "현실의 부재"가 아니라 "내 방법의 사각"일 때가 많다. *사고: "영문판 derive 수확 0"이라 단정했으나 실은 dedup 버그였고 권위 출처 2건이 묻혀 있었음.*
...
## 에이전트 운영 (Claude Code × Codex)

이 파일(`AGENTS.md`)이 정본, `CLAUDE.md`는 심링크다. 한쪽만 고쳐도 양쪽 반영.

### 역할
- **Claude Code** = 오케스트레이터 (통합·MCP·기록·핸드오프)
- **Codex** = 병렬 실행자 / 교차 리뷰어
...
예: 구현 후 `codex exec --sandbox read-only --ask-for-approval never "uncommitted diff 리뷰. 위 Source-First 등 데이터무결성 위반·정확성 이슈만 나열."`

### 병렬 (독립 태스크일 때만)
1 worktree = 1 에이전트. 같은 워킹트리에 둘 동시 금지.
`git worktree add ../kal-<task> -b <branch>`
